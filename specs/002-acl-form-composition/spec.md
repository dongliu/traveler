...
- When multiple ACL forms are attached, they are rendered in the traveler in the order they were selected during composition (a deterministic, stable ordering); an alternate ordering rule (e.g., alphabetical by title) may be adopted during planning if it proves preferable.
