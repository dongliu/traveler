...
bar, server-paged table) that reads only from the web-app endpoint and keeps the report and
add-to-binder actions on rows picked with their checkboxes (the Select all / Select none buttons
are dropped).
...
- **Visibility follows `getAccess`.** "Public" is one shared helper, `publicAccessMatch()`,
  exported from `lib/req-utils.js` next to `getAccess`. It also matches travelers with no stored
  public-access value when the configured default is public, exactly as `getAccess` treats them.
...
**Testing**: Mocha + chai + sinon. New `test/lib/public-travelers-test.js` (pure functions, `list` against a fake model, real-HTTP handler smoke tests against a tiny Express app on an ephemeral port, using Node's built-in `http`) and additions to `test/lib/csv-test.js` and `test/lib/req-utils-test.js`. Run files explicitly: the whole-directory run already fails on `ldap-client-test.js` (missing git-ignored `config/ad.json`), unrelated to this work
...
**Performance Goals**: Any page, filtered or not, and un-paged CSV in under 3 seconds for up to 5,000 public travelers (SC-001), measured first at the US1 gate and again at the end
...
**Scale/Scope**: 3 new files (lib, unit tests, API doc page) and 10 modified (`lib/csv.js` and `lib/req-utils.js` with their tests, `routes/traveler.js`, `routes/api.js`, `views/public-travelers.jade`, `public/javascripts/public-travelers.js`, `views/docs/api.jade`, `CLAUDE.md`). No model files change
...
| II. Permission-Layered Access | ✅ Pass | The listing serves only the last tier of the hierarchy (public access). The public-tier match is a shared helper, `publicAccessMatch()`, exported from `lib/req-utils.js` next to `getAccess` and imported by the lib, so the rule lives with the hierarchy and follows the same configured default `getAccess` sees (research D14). It is not caller-configurable. Routes use `auth.ensureAuthenticated` / the API's `basicAuth`, as `/publictravelers/json` does today. No per-document middleware (`exist`, `canReadMw`) applies because no single protected document is addressed, and there are no inline permission checks in route bodies. |
...
| New `lib/` functions have Mocha tests | ✅ Pass (planned) | `test/lib/public-travelers-test.js`, plus additions to `test/lib/csv-test.js` and `test/lib/req-utils-test.js`. |
| New routes have a happy-path smoke test | ✅ Pass (planned) | Real-HTTP handler smoke tests (Express parses the query string itself), plus the quickstart, which checks the route mounts and authentication. |
...
no model or config change, and no dependency. The one judgement call, where the public-visibility
rule lives, is settled by moving it into `lib/req-utils.js` beside `getAccess`.
...
├── req-utils.js               # + publicAccessMatch (the shared public-tier match, beside getAccess)
...
test/lib/
├── public-travelers-test.js   # NEW (includes the real-HTTP handler smoke tests)
├── req-utils-test.js          # + publicAccessMatch tests
└── csv-test.js                # + formula-neutralization tests

CLAUDE.md                      # + one line in the "Shared libraries" list
```
...
1. **Shared helpers**: add `publicAccessMatch` to `lib/req-utils.js` and `neutralizeFormula` /
   `toSafeCsvRow` to `lib/csv.js`, each with tests; leave `escapeCsvValue` and `toCsvRow` as they
   are so feature 001's export is unchanged.
...
6. **Validate**: run [quickstart.md](quickstart.md). The 5,000-traveler timing check runs at the
   US1 gate, before the filters, CSV, and dashboard are built on the pipeline, and again at the
   end.
...
*No constitution violations.* The one judgement call was where the public-visibility rule lives.
The listing endpoints address no single document, so per-document factories such as `canReadMw`
do not apply. Rather than leave a query predicate inside the new lib, the rule is exported as
`publicAccessMatch()` from `lib/req-utils.js`, beside `getAccess`, and unit-tested there
(research D14).
