...
- [X] T001 Confirm the baseline on branch `Ernest`: run `npx mocha test/lib/csv-test.js test/lib/req-utils-test.js` (expect 21 passing) and `npx eslint lib/csv.js` (expect no errors), so later failures can be attributed to this feature. **Note (found while implementing)**: the mocha baseline passes (21), but `npx eslint` cannot run with the project config here: ESLint 9 is installed while the config is the legacy `.eslintrc`, and `eslint-config-airbnb-base` is not installed. Lint for this feature runs through a scratch flat config that enforces the same constraints (ES2017 syntax, `no-undef`, complexity ≤ 20); Prettier 1.19 runs normally. See T043
...
- [X] T002 Create `lib/public-travelers.js` with: constants (`DEFAULT_LIMIT` 25, `MAX_LIMIT` 500, `MAX_PAGE` 100000, `MAX_TEXT_LENGTH` 200); the effective-status helper `effectiveStatus(doc)` returning `{code, name}` (archived when `doc.archived === true` or `status === 4`; names from `statusMap` exported by `model/traveler.js`; a status that is missing or not one of 0, 1, 1.5, 2, 3 counts as `initialized`, the schema default, so no traveler is ever `unknown`); and `toRecord(doc)` producing the record in data-model "Public Traveler Record" (`_id`, `title`, `status`, `statusCode`, `createdBy`, `createdOn`, `updatedBy`, `updatedOn`, `archivedOn`, `owner`, `tags`, `totalInput`, `finishedInput`, and the six classification fields). Rules: `owner || createdBy`; `archivedOn` only when `archived === true`, else `null`; `''` for missing strings, `null` for missing dates, `[]` for tags, `0` for counts; every key always present. Export the functions (FR-006, FR-007)
- [X] T003 Create `test/lib/public-travelers-test.js` (chai `should`, like `test/lib/csv-test.js`) with tests for `effectiveStatus` (archived flag over any status, status 4 without the flag, each of 0/1/1.5/2/3, a missing or unrecognized status treated as initialized) and `toRecord` (all keys present for an empty document, owner fallback, `archivedOn` hidden when not flagged even if a stale date exists, blank defaults, `statusCode` matches `status`)
- [X] T004 In `lib/public-travelers.js` add strict input readers used by all parsing: `readString(query, name)` returns a trimmed string or `''`, and throws `DataError(…, 400)` naming the parameter when the value is not a string (this rejects objects such as `subsystem[$ne]=x`); `readList(query, name)` accepts a string or an array of strings, splits on commas, trims, drops blanks, and throws `DataError` 400 for any other type. Export both
- [X] T005 In `test/lib/public-travelers-test.js` add tests for `readString` and `readList` (blank and missing → `''` / `[]`, repeated and comma-separated lists, object input rejected with status 400 and a message naming the parameter)
- [X] T006 Add and export `publicAccessMatch(defaultAccess)` in `lib/req-utils.js`, next to `getAccess`, as the single definition of the "public tier" for listings. `defaultAccess` defaults to `require('../config/config').app.default_traveler_public_access`. When that default is `0` or `1`, return `{$or: [{publicAccess: {$in: [0, 1]}}, {publicAccess: {$exists: false}}]}`: Mongoose gives a document with no stored field that default, so `getAccess` already treats it as public, while a stored `null` or `-1` stays non-public. Otherwise return `{publicAccess: {$in: [0, 1]}}`. Add a comment pointing at `getAccess` (FR-001; research D14)
- [X] T007 In `test/lib/req-utils-test.js` add tests for `publicAccessMatch`: defaults `0` and `1` include the `$exists: false` branch; defaults `-1`, `undefined`, and `''` (config not loaded) return the plain `$in [0, 1]` form; and no variant ever matches a stored `-1` or `null`
...
- [X] T023 [P] [US2] Extend `views/docs/api/public-travelers.md` with the filter parameters (`updatedFrom`, `updatedTo`, `subsystem`, `device`, `activity`, `machineArea`, `sector`, `windchillId`, `status`, `tags`, `includeArchived`), how each matches, and two or three example URLs from the contract
- [X] T024 [US2] Validate User Story 2 per `specs/004-public-traveler-dashboard/quickstart.md` §3 scenarios 4–13, on the REST endpoint and on `/publictravelers/list` (SC-004: every returned traveler satisfies all filters and no matching traveler is missing). **Result (2026-09-20)**: 56 checks pass (final run) against the throwaway MongoDB 6.0.14 through the real `routes/api.js` (real `basicAuth`, Mongoose 5.13): scenarios 4–13, plus the archived, initialized, special-character, and combined-filter cases, and the web handler returns byte-identical output to the REST route. The web route's login was not exercised (see T018)
...
- [X] T025 [P] [US3] In `lib/csv.js` add and export `neutralizeFormula(value)` (a string beginning with `=`, `+`, `-`, `@`, tab, or carriage return gets a leading single quote; non-strings, numbers, `null`, and `undefined` are returned unchanged) and `toSafeCsvRow(values)` (neutralize each value, then delegate to the existing `toCsvRow`). Do **not** change `escapeCsvValue` or `toCsvRow`, so the feature 001 export is unaffected (FR-012)
- [X] T026 [US3] In `test/lib/csv-test.js` add tests: each trigger character is prefixed; a plain string, a number, and an empty value are untouched; `toSafeCsvRow(['=SUM(A1), "draft"', 5])` yields `"'=SUM(A1), ""draft""",5`; and existing behavior is unchanged (`escapeCsvValue('=1')` is still `=1`)
- [X] T027 [US3] In `lib/public-travelers.js` extend `parseListQuery` with `format` (`json` default or `csv`, case-insensitive, else 400 `format must be json or csv`), and add `CSV_COLUMNS` (order per contract: `_id, title, status, createdBy, createdOn, updatedBy, updatedOn, archivedOn, owner, tags, totalInput, finishedInput, subsystem, device, activity, machineArea, sector, windchillId`) and `toCsv(records, options)`: header row, then one `toSafeCsvRow` per record, with tags joined by `;`, dates as ISO 8601 UTC (`''` when null), `statusCode` omitted, `\n` line endings, and a leading `\uFEFF` (the UTF-8 byte order mark) only when `options.bom` is true. No records → header only (FR-008, FR-010, FR-011, FR-014)
- [X] T028 [US3] In `lib/public-travelers.js` extend `listHandler` with the CSV branch: when `format` is `csv`, call `list` (un-paged when neither `page` nor `limit` was given, so the items pipeline has no skip/limit), then respond `200` with `Content-Type: text/csv; charset=utf-8`, `Content-Disposition: attachment; filename="public-travelers-YYYYMMDD.csv"` (request date, UTC), and the `toCsv(travelers, {bom: options.bom})` body. Errors stay JSON (FR-013)
- [X] T029 [US3] In `routes/traveler.js` change the `/publictravelers/list` mount to `publicTravelers.listHandler(Traveler, {bom: true})`, so only the web-app download carries the UTF-8 BOM; leave the REST mount in `routes/api.js` without it (research D10)
- [X] T030 [US3] In `test/lib/public-travelers-test.js` add tests for `toCsv` (header order matches the contract, no records gives the header only, comma/quote/newline escaping, formula neutralization on a title, tags joined with `;`, ISO dates, null dates empty, `statusCode` absent, BOM present only with `bom: true`) and for the handler's CSV branch as real HTTP with the T014 helper (`Content-Type` and `Content-Disposition` headers, a body that starts with the header row, un-paged when no `page`/`limit`, paged when `limit` is given, a leading BOM only when the handler is created with `{bom: true}`, `format=xml` → 400 JSON error)
- [X] T031 [P] [US3] Extend `views/docs/api/public-travelers.md` with the CSV format: `format=csv`, the column list, paging behavior (un-paged returns everything), tag separator, timestamp format, and formula protection
- [X] T032 [US3] Validate User Story 3 per `specs/004-public-traveler-dashboard/quickstart.md` §3 scenarios 16–19, and open the downloaded CSV in a spreadsheet application to confirm columns align, the comma/quote title stays in one cell, and the `=SUM…` title shows as text (SC-006). **Result (2026-09-20)**: 23 checks pass (final run) on the throwaway MongoDB 6.0.14 through the real REST route and the web handler: scenarios 16–19, filters and paging applied to the CSV, `status=archived`, the errors staying JSON, and the web file being the REST file plus exactly the BOM. The files were read with Python's `csv` module (an independent RFC 4180 reader): every row has 18 cells, the title with a comma, quotes, and a line break stays in one cell, international text is intact, and no cell starts as a formula. **No spreadsheet application was opened**, so SC-006's "opens in Excel/Numbers" is not confirmed by eye
...
- [X] T039 [US5] In `views/public-travelers.jade` add the `#status-summary` container above the filter bar and an inline `style.` block modeled on `views/ncr-dashboard.jade` (`.status-card`, `.status-card.active`, `.status-card .count`), with one background color per status (initialized, active, submitted for completion, completed, frozen, archived) (FR-028)
- [X] T040 [US5] In `public/javascripts/public-travelers.js` render the cards from `statusCounts` on every load: an **All** card whose count is the sum of the counts shown, then one card per status, with the **archived** card present only while include-archived is checked. Clicking a card sets `state.status`, resets to page 1, highlights it, and reloads (clicking All or the active card clears it); send `status` in `buildQuery()`. The counts must not change when a status is selected. Unchecking include-archived while the archived card is selected clears the selection. Reuse the same color map for the status badge column from T034 (FR-028)
- [X] T041 [US5] Validate User Story 5 per `specs/004-public-traveler-dashboard/quickstart.md` §4 steps 1, 3, and 5: card counts, selecting a card narrows the table and keeps other counts, counts sum to All for several filter combinations (SC-009), and the archived card appears only with the checkbox. **Result (2026-09-20)**: 31 checks pass (final run) in the same real headless Chromium against the real page, handler, and MongoDB: the six cards and their counts (14 / 16 / 12 / 14 / 12, with a traveler that has no status counted as initialized), selecting, switching, and clearing a card, the counts staying put while a status is selected and following every other filter, the counts adding up to All for a date range, a tag, subsystem plus Windchill, no match, and with a status selected (SC-009), the archived card appearing only with Include archived (All 70, archived 2) and going away with a selection of it when unchecked, the badge colour matching the card colour, the CSV honouring the selected status, and a slow out-of-date answer not changing the cards. The US4 walkthrough was re-run afterwards with no regression
...
- [X] T042 [P] Add `public-travelers.js` to the "Shared libraries (`/lib/`)" list in `CLAUDE.md`, with one line describing it (shared listing used by the web route and the REST route)
...
- [X] T044 Run `npx mocha test/lib/public-travelers-test.js test/lib/csv-test.js test/lib/req-utils-test.js` and confirm everything passes
- [X] T045 Re-run the SC-001 timing from T018 with everything built: page 1, a filtered page, and un-paged CSV (`format=csv`) on the 5,000-traveler data set, each under 3 seconds (SC-001), and confirm no "Sort exceeded memory limit" in the log. If it fails, apply the escape hatch in `research.md` D4/D5 instead of adding an ad-hoc index **Result (2026-09-20)**: end to end over real HTTP through the real REST route and Mongoose, on the 5,000-traveler set (about 40 KB each, 4,258 public): page 1 about 19 ms, page 80 at 50 per page about 21 ms, 500 per page about 23 ms, three filtered pages 7–20 ms, un-paged CSV of every match (4,258 rows, 737 KB) about 53 ms, filtered un-paged CSV about 22 ms; worst single request 64 ms against the 3-second budget, and no memory-limit error. Local and warm, so an upper bound on the pipeline's own cost rather than a production prediction
- [X] T046 Security pass over the diff in `lib/public-travelers.js`, `lib/csv.js`, `routes/traveler.js`, `routes/api.js`, `lib/req-utils.js`, and `public/javascripts/public-travelers.js`: no non-public traveler can appear (`publicAccessMatch()` is applied unconditionally in `buildBaseMatch`), object-valued query parameters are rejected, every user-authored value in the dashboard is escaped, CSV formula protection is on for all text cells, no `console.log`, and error responses never echo internal messages (SC-005). Optionally run `/security-review` **Result (2026-09-20)**: the pass found and fixed **one real defect**: a null character in any text parameter (`subsystem=%00`) made MongoDB refuse the regular expression and the listing answered **500**, against FR-021. `readString` and `readList` now reject it with a 400 naming the parameter (tests added, including the real-HTTP case). Also checked: the public-tier match is the first condition of both pipelines and no parameter can change it (unit tests, and the real database with private, `null`, missing, and archived travelers); bracket, array, and prototype-style parameters (`status=__proto__`, `tags[$ne]=a`) are rejected or harmless; regex-special text is literal; a 1,000-tag request takes 25 ms and an oversized query is refused by Node itself (431); `page=100000&limit=500` takes 20 ms; a 1,500-query randomized fuzz over the real route plus 11 raw malformed-UTF-8 requests produced only 200s and clean JSON 400s, never a 500 and no internal text; every value in the dashboard is escaped (a browser test with a hostile title, subsystem, tag, owner, and updated-by ran nothing); CSV formula protection covers every text cell (a cell that starts with a space and then `=` is not neutralized: the usual guidance covers only the first character, and I did not check how every spreadsheet application treats a leading space); no `console.*` or secrets in the new code; error bodies never echo internal messages. Not run: `/security-review`, which this task lists as optional
- [X] T047 Run the whole of `specs/004-public-traveler-dashboard/quickstart.md` end to end (§1–§6) on a clean seed and record any deviations **Result (2026-09-20)**, every scenario run in quickstart order, each from a freshly reseeded database: §1 187 unit and real-HTTP tests pass, Prettier clean, no lint findings in the new files. §3 scenarios 1–23: US1 30 checks, US2 56, US3 23, plus scenarios 13 (null character) and 14 (3), and scenario 15 with the real REST route (13), all pass. §4 the dashboard walkthrough (53) and the status cards (31) pass in a real Chromium. §6 timings pass (worst 64 ms). A 1,500-query randomized fuzz plus raw malformed-UTF-8 requests gave no 500s. **Deviations**: (1) the checks ran against a throwaway MongoDB 6.0.14 (`mongodb-memory-server`, in the scratchpad) and a scratch server that bypasses login and stubs `/travelers/report/` and `/binders/:id/`, not the full `node app.js` (which needs the LDAP/CAS and API configuration), so the web route's `ensureAuthenticated` is confirmed only by its registration; (2) no spreadsheet application was opened, so "opens correctly in Excel/Numbers" (SC-006) is unconfirmed by eye; (3) SC-007's 30 seconds was not timed by a person; (4) the project's own `npx eslint` cannot run here (ESLint 9 with a legacy `.eslintrc`, and `airbnb-base` not installed), so a scratch config with the same constraints was used, and the quickstart now says so; (5) all timings are on local, warm data. **One defect found and fixed along the way** (a null character in a filter gave a 500; see T046)
