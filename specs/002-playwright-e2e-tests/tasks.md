...
- [x] T012 Create `e2e/playwright.config.js`: sets `baseURL` from `e2e/fixtures/env.js`'s resolved `WEB_PORT`, `globalSetup: './global-setup.js'`, a single top-level `use.storageState` defaulting every spec file to the primary persona (`e2e/fixtures/auth-state.js`'s `PRIMARY_AUTH_STATE`) — deliberately not split into `primary`/`secondary` *projects*, since Playwright runs every matched file under every project, which would double-run every story's spec file and create a real race condition on shared-state tests (e.g. US1's ncr-qa group mutation) if two projects' workers hit it concurrently under `fullyParallel`; the few scenarios needing the secondary identity instead opt in locally via `test.use({ storageState: SECONDARY_AUTH_STATE })` — `trace: 'retain-on-failure'`, `video: 'retain-on-failure'`, `screenshot: 'only-on-failure'`, and both the HTML reporter (default) and JSON reporter (output to `playwright-report/results.json`) per research.md Decision 6; fails fast with a clear message if `baseURL` or the Mailpit URL is unreachable (a `globalSetup` connectivity check satisfying FR-012)
...
- [x] T013 [US1] In `e2e/us1-create-and-submit-ncr.spec.js`, write the NCR creation and field-validation tests (spec Acceptance Scenarios 1–2): submit the creation form with all mandatory fields (using `run-id.js` to scope `part_number`/`wbs_number` uniquely) and assert the resulting NCR number and "Submitted" status; separately submit with missing/invalid mandatory fields and assert field-level validation messages appear and no NCR is created
- [x] T014 [US1] In the same file, write the CE/CS disposition-request email test (Acceptance Scenario 3): after creating an NCR with a designated CE/CS, use `e2e/fixtures/mailpit.js`'s `waitForMessage` to find the email, and assert its `To` contains the CE/CS address, its `Cc` contains the Originator's address, and its body (via `getMessage`) contains the NCR number, part name, supplier, and originator name
- [x] T015 [US1] In the same file, write the QA Admin initial-notification email test (Acceptance Scenario 4): find the email via `mailpit.js`, assert `To` contains each `ncr-qa` group member's address (read via `exec-cli.js`'s `get-group`), `Cc` contains the Originator's address, and the body contains the NCR number, part name, supplier, originator name, CE/CS name, and problem description
- [x] T016 [US1] In the same file, write the event-log delivery-tracking test (Acceptance Scenario 5): use `exec-cli.js`'s `get-ncr` (requesting the `events` field) to assert both the `notification.disposition_request` and `notification.initial` events have a TO entry with `delivery_status`/`delivery_timestamp` populated and a separate CC entry (the Originator) with its own `delivery_status`/`delivery_timestamp`
- [x] T017 [US1] In the same file, write the ncr-qa-misconfigured failure-path test (Acceptance Scenario 6, FR-007a): use `exec-cli.js`'s `remove-group-member` to empty the `ncr-qa` group, attempt NCR creation, assert the submission fails with the configured error message, and assert via a Mailpit before/after count that the CE/CS disposition-request email was also not sent (FR-007a: no email — including the CE/CS one — is sent when QA staff isn't configured), then restore the group's prior membership via `add-group-member` in a `finally`/cleanup block so this test never leaves the group empty for other scenarios
...
- [ ] T020 [US2] In the same file, write the QA-concurrence-with-approver test (Acceptance Scenario 3): record concurrence designating the **secondary** persona's user id as an additional approver, and assert the NCR reaches "Approval Requested" with an approval-request notification recorded for that approver
- [ ] T021 [US2] In the same file, write the return-for-comment / resubmit round-trip test (Acceptance Scenario 4): using `test.use({ storageState: SECONDARY_AUTH_STATE })` (the approver's own session) for the return-for-comment step, return an Approval Requested NCR for comment and assert it reaches "Returned for Comment"; then, back under the default primary-persona session as QA, resubmit and assert it returns to "Approval Requested"
