...
| Traveler page | `.copy-ncr-ref` (copy button), `.initiate-ncr-link` (existing), `.ncr-link-badge` (existing), `.ncr-links-existing` (warning box listing an input's NCRs), `.ncr-pdf-link` (PDF link), `#complete2` ("Submit for completion"), `#finished-input` (existing) |
...
- [X] T047 Run `npx eslint .` and fix every new violation in files this feature touched (in particular unused imports/variables left by removing `findLabelForInput` and the banner script).
- [X] T048 Run `TRAVELER_CONFIG_REL_PATH=docker npm run unit` and confirm the whole suite is green; then measure coverage of the two new lib files with `TRAVELER_CONFIG_REL_PATH=docker npx c8 --include lib/traveler-ncr.js --include lib/ncr-pdf.js npx mocha test-unit/lib/traveler-ncr.test.js test-unit/lib/ncr-pdf.test.js` and add tests until each is ≥ 80% (constitution I).
- [X] T049 [P] Run `npm audit --omit=dev` and record any advisory touching `pdfkit`, `dejavu-fonts-ttf` or their dependencies in the PR description (constitution III).
- [X] T050 Rebuild the stack (`docker compose up --build`) and confirm the new dependencies and fonts work in the `node:20-alpine` image: `docker compose exec web node -e "const {buildNcrPdfModel,renderNcrPdf}=require('./lib/ncr-pdf'); renderNcrPdf(buildNcrPdfModel({ncr_number:'NCR-T',description_of_nonconformance:'4.7 Ω ≥ 5 Ω'})).then(b=>console.log(b.slice(0,5).toString(), b.length))"` prints `%PDF-` and a nonzero length. **Verification note:** `docker compose up --build` could not complete here — Docker Desktop could not reach Docker Hub to resolve `node:20-alpine` (the build hung at "load metadata"). Verified instead inside the running alpine/Node 20.20 `web` container: a clean `npm ci --omit=dev` from the updated `package-lock.json` resolves `pdfkit` 0.20.2 and both DejaVu fonts, `PDFDocument` constructs, and `lib/ncr-pdf.js` rendered a PDF with `Ω ≥ Δ µ`. The 19 new pure-JS packages were then copied into that container's `node_modules` volume so the e2e suite could run; a normal image rebuild will install them itself.
- [X] T051 With the stack running, run the full e2e suite `npm run e2e` and confirm `e2e/us-traveler-ncr-gating.spec.js` and `e2e/us-traveler-ncr-input-linking.spec.js` pass and no other NCR spec regressed. **Result:** run serially (`--workers=1`) the whole suite is 123 passed / 1 failed. The one failure, `e2e/us2-ce-cs-disposition.spec.js` AS1, is pre-existing and unrelated: it clicks `a:has-text("Submit Disposition")`, but commit 1a3d4d27 (#228) renamed that button to "Create Disposition" in `views/ncr-detail.jade` and the test was never updated. With the default parallel workers, 3–4 further tests fail per run but a *different* set each time (`us-originator-designate`, `us-ncr-attachments`, `us3-…`), which pass when run alone or serially — cross-spec interference on shared user/group state and slow email sending under load (`e2e/playwright.config.js` already warns about shared state).
- [X] T052 Walk through every step of `specs/124-traveler-input-ncr-gating/quickstart.md` manually, including step 3.6 (the two REST API routes — set a traveler's status to 1.5, and to 2 while active, with an open NCR, using the Basic-auth API user) and step 5.6 (PDF failure path). **How this was done:** no literal manual browser session. Every quickstart step is covered by an e2e test in `e2e/us-traveler-ncr-gating.spec.js` (steps 3.5 uses `set-traveler-status` to send the traveler back rather than a reviewer rejection, which is equivalent because only submission is gated), and the pieces e2e cannot see were checked directly: step 3.6 (both Basic-auth REST routes) against the live API with the stack's `api_write` user, then made permanent as an e2e test; step 4's binder rollup against real Mongoose models, then made permanent; and step 5.3 by closing a real NCR through the live app, downloading its PDF and rendering it (traveler title/id, the `Ω` in the input label, originator and CE/CS names, and `Ω ± ≥ Δ µ é ü` in the description all render correctly).
- [X] T053 Add a `CLAUDE.md` "Recent Changes" entry for `124-traveler-input-ncr-gating` recording: the new dependencies (`pdfkit`, `dejavu-fonts-ttf`); the new collection `TravelerNcrPdf`; the `409 OPEN_NCRS` behaviour on `PUT /travelers/:id/status`, `PUT /apis/travelers/:id/status/` and `POST /apis/update/traveler/:id/`; and the deprecated `traveler_id`/`traveler_input_name`/`traveler_input_label` body fields on `POST /api/ncrs` (client label ignored, `traveler_id` without an input name now 400). (constitution IV)
...
- The REST API status routes (`/apis/...`, Basic auth as `api_write`) are not reachable by the e2e *personas*, so they are driven with their own request context that reads the credentials from the stack's `docker/api.json` (see the last test in `e2e/us-traveler-ncr-gating.spec.js`), plus the `isSubmissionTransition` unit test (T020).
