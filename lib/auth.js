...
function ensureAuthenticated(req, res, next) {
  ...
  if (req.get('x-forwared-host') && req.get('x-forwared-host') === auth.proxy) {
    req.proxied = true;
  }
  if (req.session.userid) {
    ...
    if (req.query.ticket) {
      ...
      return res.redirect(301, url.format({
        pathname: req.proxied ? path.join(auth.proxied_service, ticketUrl.pathname) : ticketUrl.pathname,
          ...
          User.findOne({
            ...
            if (user) {
              ...
              if (req.session.landing) {
                return res.redirect(req.proxied ? path.join(auth.proxied_service, req.session.landing) : req.session.landing);
              }
              next();
              halt.resume();
            } else {
