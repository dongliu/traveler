...
function redirectService(res, req, destination) {
  ...
  if (req.proxied) {
    return res.redirect(auth.proxied_service + destination);
  } else {
...
function redirectToLoginService(req, res) {
  if (auth.type === 'cas') {
    ...
    if (req.xhr) {
      ...
      if (req.proxied) {
        return res.redirect(auth.proxied_cas + '/login?service=' + encodeURIComponent(auth.login_proxied_service));
      } else {
    ...
    User.findOne({
      ...
      if (user) {
        ...
        if (ad.groupSearchBase === undefined) {
          ...
          }).lean().exec(function (groupErr, group) {
            if (groupErr) {
