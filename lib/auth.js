        ...
        if (ldapLookup) {
            ldapClient.searchForUser(username, function(err, ldapUser) {
                if (err !== null) {
                    ...
                    authenticateUsingLdap(ldapUser.dn, username, password, req, res, next)
                }
            });
        }
        else {
            var bindDN = 'uid=' + username + ',' + baseDN;
            authenticateUsingLdap(bindDN, username, password, req, res, next)
        }
    }
}
