...
// Returns { results, ccResults } where ccResults tracks CC delivery (best-wins across multiple TO sends)
async function sendToRecipients(subject, html, recipients, cc) {
  ...
  const results = [];
  const ccBest = {}; // keyed by CC email; once Delivered it is never overwritten by a failure

  for (const email of emails) {
    let ok;
    let sendErr;
    try {
      ok = await sendNotification({ subject, html, recipients: email, cc: ccEmails.length ? ccEmails.join(',') : undefined });
    ...
    for (const ccEmail of ccEmails) {
      if (!ccBest[ccEmail] || ccBest[ccEmail].delivery_status !== 'Delivered') {
        ccBest[ccEmail] = buildResult(ccEmail, delivered, sendErr);
      }
    }
  }

  return { results, ccResults: Object.values(ccBest) };
}
