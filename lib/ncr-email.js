...
async function sendInitialNotification(ncr, recipients, ncrUrl, originatorEmail) {
  ...
  const linkHtml = ncrUrl
    ? `<p>You can get current, as well as, on-going details by following the link below:</p>
    <p><a href="${ncrUrl}">View NCR ${ncr.ncr_number}</a></p>`
    : '';
  const html = `
...
async function sendQaNotification(ncr, qaStaff, context, ncrUrl) {
  ...
  if (context === 'returned_for_comment') {
    ...
    body = `
    <p>QA Admin:</p>
    <p>The CE/CS section of Nonconformance Report No. ${ncr.ncr_number}, concerning the ${ncr.part_name}
    manufactured by ${ncr.supplier_name}, has been completed by ${ncr.ce_cs_name}.
    Please complete the QA section of the report. You can do so by following the link below:</p>`;
  }
  const html = `
    ${body}
    ${linkHtml}
  `;
  const { results } = await sendToRecipients(subject, html, qaStaff);
  return results;
}

async function sendApprovalRequest(ncr, approvers, ncrUrl, approvedByName) {
  const subject = `Action Required — Approval Needed for NCR ${ncr.ncr_number}`;
  const linkHtml = ncrUrl
    ? `<p><a href="${ncrUrl}">View NCR ${ncr.ncr_number} and submit approval</a></p>`
    : '';
  const html = `
    <p>Approvers:</p>
    <p>Nonconformance Report No. ${ncr.ncr_number}, concerning the ${ncr.part_name} manufactured by
    ${ncr.supplier_name}, has been approved by ${approvedByName || 'QA Admin'}, and is ready for
    your approval. You can review and/or approve this report by following the link below:</p>
    ${linkHtml}
  `;
  const { results } = await sendToRecipients(subject, html, approvers);
  return results;
}

async function sendIssuance(ncr, originators, ncrUrl) {
  const subject = `NCR ${ncr.ncr_number} — Final Approval Reached`;
  const linkHtml = ncrUrl
    ? `<p>You can close the report by following the link below:</p>
    <p><a href="${ncrUrl}">View NCR ${ncr.ncr_number} and close the report</a></p>`
    : '';
  const html = `
    <p>${ncr.originator_name}:</p>
    <p>Nonconformance Report No. ${ncr.ncr_number}, concerning the ${ncr.part_name} manufactured by
    ${ncr.supplier_name}, has been approved by all associated members. Please execute the parts
    disposition. When parts disposition is completed, please close out the report.</p>
    ${linkHtml}
  `;
  const { results } = await sendToRecipients(subject, html, originators);
  return results;
}

async function sendFinalDistribution(ncr, recipients, ncrUrl) {
  const subject = `NCR ${ncr.ncr_number} — Closed`;
  const linkHtml = ncrUrl
    ? `<p>You can view this report by following the link below:</p>
    <p><a href="${ncrUrl}">View NCR ${ncr.ncr_number}</a></p>`
    : '';
  const html = `
    <p>Nonconformance Report No. ${ncr.ncr_number}, concerning the ${ncr.part_name} manufactured by
    ${ncr.supplier_name}, has been closed.</p>
    ${linkHtml}
  `;
  const { results } = await sendToRecipients(subject, html, recipients);
  return results;
}
...
async function sendPaAssigned(ncr, pa, owner) {
  ...
  `;
  const { results } = await sendToRecipients(subject, html, [owner.email]);
  return results;
}

async function sendDesignateAssigned(ncr, designateEmail, ncrUrl) {
  const subject = `You Have Been Assigned as Designate — NCR ${ncr.ncr_number}`;
  const linkHtml = ncrUrl
    ? `<p><a href="${ncrUrl}">View NCR ${ncr.ncr_number}</a></p>`
    : '';
  const html = `
    <p>${ncr.originator_name} has assigned you as Designate for Nonconformance Report
    No. ${ncr.ncr_number}, concerning ${ncr.part_name} manufactured by ${ncr.supplier_name}.</p>
    <p>As Designate, you may act on this NCR with the same authority as the Originator,
    including closing it once it reaches Final Approval.</p>
    ${linkHtml}
  `;
  const { results } = await sendToRecipients(subject, html, [designateEmail]);
  return results;
}
