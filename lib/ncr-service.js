...
async function createNcr(data, user, webBaseUrl) {
  ...
  const initialResults = await sendInitialNotification(ncr, qaEmails, ncrUrl);
  appendNotificationEvent(ncr, 'notification.initial', initialResults);

  if (data.ce_cs_email) {
    const dispResults = await sendDispositionRequest(ncr, data.ce_cs_email, ncrUrl);
    appendNotificationEvent(ncr, 'notification.disposition_request', dispResults);
  }
