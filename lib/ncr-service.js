...
async function createNcr(data, user) {
  ...
  const initialResults = await sendInitialNotification(ncr, originatorDoc ? originatorDoc.email : '');
  appendNotificationEvent(ncr, 'notification.initial', initialResults);

  if (data.ce_cs_email) {
    const dispResults = await sendDispositionRequest(ncr, data.ce_cs_email);
