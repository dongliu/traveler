...
async function submitConcurrence(ncrId, additionalApprovers, user, webBaseUrl) {
  ...
  if (approvers.length === 0) {
    ...
    // displayName), not looked up in the local User collection here --
    // a designated approver may never have logged into the app and so may
    // have no local User record at all. findUsers is only still needed
    // below to look up email addresses for the approval-request notification.
    ...
    const results = await sendApprovalRequest(ncr, approverUsers.map(u => u.email), ncrUrl, user.name);
