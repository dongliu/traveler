...
async function createNcr(data, user, webBaseUrl) {
  ...
  const [originatorDoc] = await findUsers([user.id]);
  const initialResults = await sendInitialNotification(ncr, originatorDoc ? originatorDoc.email : '', ncrUrl);
...
async function updatePaStatus(ncrId, paId, data, user) {
  ...
  if (pa.owner_id !== user.id && !hasRole(user, 'qa_staff')) {
