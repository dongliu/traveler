...
async function submitDisposition(ncrId, data, user) {
  ...
  const qaResults = await sendQaNotification(ncr, qaUsers.map(u => u.email));
...
async function submitConcurrence(ncrId, additionalApprovers, user) {
  ...
  if (approvers.length === 0) {
    ...
    const results = await sendIssuance(ncr, issuanceUsers.map(u => u.email));
    ...
    const results = await sendApprovalRequest(ncr, approverUsers.map(u => u.email));
...
async function returnForComment(ncrId, comments, user) {
  ...
  if (qaUsers.length > 0) {
    const results = await sendQaNotification(ncr, qaUsers.map(u => u.email), 'returned_for_comment');
