const nodemailer = require('nodemailer');
const { convert: htmlToText } = require('html-to-text');

const config = require('../config/config');

// this is a hacking way to test and such code should go to either the test or docker dev environment
if (require.main === module) {
  config.load();
}
const { app } = config;

const transport = nodemailer.createTransport({
  host: app.smtp_host,
  port: app.smtp_port,
  secure: app.smtp_ssl, // upgrade later with STARTTLS
  ignoreTLS: !app.smtp_tls,
});

/**
 * Send an email notification to one or more recipients.
 *
 * @param {{subject: string | undefined, recipients: string | string[], text: string | undefined, html: string | undefined}} options
 * @param {string} options.subject The subject of the email (optional)
 * @param {(string|string[])} options.recipients A single email address or an array of email addresses
 * @param {string} options.text The plain text body of the email (optional)
 * @param {string} options.html The HTML body of the email (optional)
 */
function sendNotification({ subject, recipients, text, html }) {
  if (Array.isArray(recipients) && recipients.length === 0) {
    return;
  }
  if (text == null && html != null) {
    text = htmlToText(html);
  }
  transport.sendMail({
    // TODO: this should be a config of the mail service
    from: '"eTraveler Notification" <notification@exmaple.com>',
    to: recipients,
    subject:
      subject != null
        ? `eTraveler Notification - ${subject}`
        : 'New eTraveler Notification',
    text,
    html,
  });
}

module.exports = { sendNotification };

// this is a hacking way to test and such code should go to either the test or docker dev environment
if (require.main === module) {
  sendNotification({
    subject: 'Hello!',
    html: "<h1>This is a demo</h1><br/><a href='http://google.com'>Link</a>",
    recipients: 'test@example.com',
  });
}
