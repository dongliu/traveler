...
function addUserFromAD(req, res, doc) {
  ...
  ldapClient.search(ad.searchBase, opts, false, function (err, result) {
    ...
    doc.save(function (docErr) {
      ...
      return res.status(201).send('The user named ' + name + ' was added to the share list.');
    });
  });
}
...
function addGroupFromAD(req, res, doc) {
  ...
  ldapClient.search(ad.groupSearchBase, opts, false, function (err, result) {
    ...
    doc.save(function (docErr) {
      ...
      return res.status(201).send('The group ' + id + ' was added to the share list.');
    });
  });
}
...
function removeFromList(req, res, doc) {
  ...
  if (removed.length === 0) {
    return res.status(400).send('cannot find ' + req.params.shareid + ' in list.');
  }
...
function addShare(req, res, doc) {
  if (['Form', 'Traveler', 'Binder'].indexOf(doc.constructor.modelName) === -1) {
    return res.status(500).send('cannot handle the document type ' + doc.constructor.modelName);
  }
  if (req.params.list === 'users') {
    addUser(req, res, doc);
  }
