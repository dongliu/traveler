  ...
  if (type === 'Form') {
    if (doc.status !== 0.5) {
      ...
      try {
        ...
        logger.error(`failed to save form ${doc._id} status on rejection: ${err}`);
        throw err;
      }
    }
  }
  if (type === 'Traveler') {
    if (doc.status !== 1.5) {
      ...
      try {
        ...
        logger.error(`failed to save traveler ${doc._id} status on rejection: ${err}`);
        throw err;
      }
    }
  }
...
async function removeReviewRequest(req, res, doc) {
  ...
  try {
    ...
    const newDoc = await doc.removeReviewRequest(ids);
    if (newDoc.allApproved()) {
      ...
      sendNotification({
        recipients: owner.email,
        subject: 'Review Complete',
        text: `The review requests have been removed for the document: ${
          newDoc.title
        }.\nGo to this link to view the document: ${req.protocol}://${req.get(
          'host'
        )}/forms/${
          newDoc._id
        }/\nThe template was released because all the remaining reviewers have approved it.`,
        html: `The review requests have been removed for the document: ${
          newDoc.title
        }<br/>Go to this link to view the document: <a href="${
          req.protocol
        }://${req.get('host')}/forms/${newDoc._id}/">${
          req.protocol
        }://${req.get('host')}/forms/${
          newDoc._id
        }/</a><br/>The template was released because all the remaining reviewers have approved it.`,
      });
      return releaseForm(req, res);
    }
...
async function addReviewResult(req, res, doc) {
  const { result = '2', comment, v, reviewerId: onBehalfOf } = req.body;
  const reviewerId = onBehalfOf || req.session.userid;
  if (onBehalfOf) {
    logger.info(
      `admin ${req.session.userid} submitted review on behalf of reviewer ${onBehalfOf} for ${doc.constructor.modelName} ${doc._id}`
    );
  }
  try {
    ...
    if (newDoc.allApproved()) {
      return await allApproveFlow(owner, reviewer, verb, comment, newDoc, req, res);
    }
    if (result == '2') {
      return await rejectionFlow(owner, reviewer, verb, comment, newDoc, req, res);
    }
