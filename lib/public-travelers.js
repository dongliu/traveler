 ...
 * archived, since the date is not cleared when a traveler is restored.
 * @param  {Object} doc a traveler (document or plain object)
...
function buildBaseMatch(params) {
  ...
  TEXT_FILTERS.forEach(function add(name) {
    if (textFilters[name]) {
      conditions.push({
        [name]: { $regex: _.escapeRegExp(textFilters[name]), $options: 'i' },
      });
    }
  });
...
function csvCell(record, column) {
