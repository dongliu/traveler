...
async function releaseForm(req, res) {
  ...
  if (req[req.params.id].formType === 'normal' && req.body.discrepancyFormId) {
    reqUtils.existSource(
      'discrepancyFormId',
      'body',
      ReleasedForm
    )(
  ...
  if (
    ...
    return res
      .status(400)
      .send(`${req[req.body.discrepancyFormId].id} is not a discrepancy form`);
  }
  ...
  await form.saveWithHistory(req.session.userid);
  ...
  // carry over the review info if there is any, so that the released form will
  // have the same review history as the form, and the reviewer can see the
  // review history in the released form.
  releasedForm.__review = form.__review;
  if (discrepancyForm) {
    // update formType
  ...
  try {
    ...
    // after upton change, the following line does nothing
