...
const htmlSanitizeConfiguration = {
  ...
  allowedAttributes: {
    ...
    tbody: ['class', 'style'],
    tr: ['class', 'style'],
    th: ['class', 'style', 'data-cell-type', 'colspan', 'rowspan', 'scope'],
    td: ['class', 'style', 'data-cell-type', 'colspan', 'rowspan'],
  },
};
...
/**
 * The stored publicAccess values that make a document public: 0 (read) and
 * 1 (write). Mirrors the publicAccess checks at the end of getAccess.
 */
const PUBLIC_ACCESS_VALUES = [0, 1];

function isPublicAccessValue(value) {
  if (value === undefined || value === null || value === '') {
    return false;
  }
  return PUBLIC_ACCESS_VALUES.indexOf(Number(value)) !== -1;
}

/**
 * MongoDB match for the "public tier" of the permission hierarchy, for code
 * that lists documents (a query cannot call getAccess per document). Keep it
 * in step with getAccess:
 * - a stored publicAccess of 0 or 1 is public; -1 and null are not;
 * - Mongoose gives a document with no stored publicAccess the configured
 *   default (default_traveler_public_access) when it is loaded, so getAccess
 *   sees that default. When the default is public, such a document is public
 *   here too, hence the $exists: false branch. It is $exists, not null, since
 *   Mongoose does not apply the default to a stored null.
 * @param  {Number} [defaultAccess] the default for documents with no stored
 *   publicAccess; defaults to the configured value
 * @return {Object}               a fresh match object
 */
function publicAccessMatch(
  defaultAccess = config.app && config.app.default_traveler_public_access
) {
  const stored = { publicAccess: { $in: PUBLIC_ACCESS_VALUES.slice() } };
  if (!isPublicAccessValue(defaultAccess)) {
    return stored;
  }
  return { $or: [stored, { publicAccess: { $exists: false } }] };
}
