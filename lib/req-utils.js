...
const { Manager, Admin } = require('./role');

const { Read_all_forms, Write_active_travelers } = require('./permission');
...
function isMw(type) {
  return function(req, res, next) {
    ...
    return res.status(400).send(`expect ${type} content type`);
  };
}
...
function hasAll(list, names) {
  return function(req, res, next) {
    ...
    for (i = 0; i < names.length; i += 1) {
      if (!Object.hasOwn(req[list], names[i])) {
        miss = true;
        missed = names[i];
        break;
      }
...
function getAccess(req, doc) {
  ...
  if (
    ...
    routesUtilities.hasPermission(req, Write_active_travelers)
  ) {
    return 1;
  }
  if (
    doc.constructor.modelName === 'Form' &&
    routesUtilities.hasPermission(req, Read_all_forms)
  ) {
    return 0;
  }
...
function isAdmin(req) {
  const role = Admin;
  if (req && req.res && req.res.locals && Array.isArray(req.res.locals.roles)) {
    return req.res.locals.roles.indexOf(role) !== -1;
  }
  return false;
}
...
function isOwnerOrAdminMw(pName) {
  return function(req, res, next) {
    if (isOwner(req, req[req.params[pName]]) || isAdmin(req)) {
      return next();
    }
    return res
      .status(403)
