...
var htmlSantitizeConfiguration = {
  allowedTags: [
    'div',
    'a',
    'legend',
  ...
  allowedAttributes: {
    ...
    'legend': ['id'],
    'label': ['class'],
    'input': ['type', 'disabled', 'name', 'placeholder', 'required', 'data-userkey', 'value', 'min', 'max'],
    'img': ['name', 'alt', 'width', 'src'],
    'textarea': ['disabled', 'name', 'placeholder', 'rows', 'required', 'data-userkey'],
    'a': ['href', 'title', 'target']
  }
};
    ...
    names.forEach(function (n) {
      if (req[list].hasOwnProperty(n)) {
        if (_.isString(req[list][n])) {
          req[list][n] = sanitize(req[list][n], htmlSantitizeConfiguration);
        }
