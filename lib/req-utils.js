...
const { Traveler } = require('../model/traveler.js');
...
function getAccess(req, doc) {
  ...
  if (doc.sharedWith && doc.sharedWith.id(userid)) {
    return doc.sharedWith.id(userid).access;
  }
  var i;
  if (doc.sharedGroup && memberOf) {
