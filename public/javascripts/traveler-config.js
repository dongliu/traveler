...
$(function () {
  ...
  $('#add-tag').click(function (e) {
    ...
    $('#tags').append('<li><form class="form-inline"><input id="new-tag" type="text"> <button id="tag-confirm" class="btn btn-primary">Confirm</button> <button id="cancel" class="btn">Cancel</button></form></li>');
    $('#cancel').click(function (cancelE) {
      cancelE.preventDefault();
      cleanTagForm();
    });
    ...
    $('#tag-confirm').click(function (confirmE) {
      ...
      if ($('#new-tag').val().trim()) {
        $.ajax({
          ...
          if (jqXHR.status === 200) {
            $('#tags').append('<li><span class="tag">' + data.tag + '</span> <button class="btn btn-small btn-warning remove-tag"><i class="fa fa-trash-o fa-lg"></i></button></li>');
          }
        }).fail(function (jqXHR) {
          if (jqXHR.status !== 401) {
            $('#message').append('<div class="alert alert-error"><button class="close" data-dismiss="alert">x</button>Cannot add the tag</div>');
