...
export function dearchiveFromModal(type, ...tables) {
  $('#submit').prop('disabled', true);
  $('#return').prop('disabled', true);
  let number = $('#modal .modal-body div.target').length;
  $('#modal .modal-body div.target').each(function() {
...
// de-archive the selected item to its original status
export function dearchive(type, fromTable, ...otherTables) {
  const selected = fnGetSelected(fromTable, 'row-selected');
  modalScroll(false);
  if (selected.length === 0) {
    noneSelectedModal();
    ...
    $('#modal').modal('show');
    $('#submit').click(function() {
      dearchiveFromModal(type, fromTable, ...otherTables);
    });
  }
}
